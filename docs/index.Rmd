...
The document includes the source code used in the analysis. Choose `Hide All
Code` option from the top right corner if you do not desire to see the code
chunks.
...
compact <- as_tibble(compact)
head(compact)
```

We abbreviate and therefore recode form topics because they seem to be too long
...
recode.lvls <- sapply(unique(compact$labelFormsRecode), function(x) do_abb(x))
levels(compact$labelFormsRecode) <- recode.lvls
stopifnot(anyDuplicated(levels(compact$labelFormsRecode)) == 0)
...
Number of Capitals and Provinces

```{r}
compact %>% count(capital, province, sort = TRUE) %>% knitr::kable()
```
...
The response word count distribution per form topic categorized by capital name:
...
cp %>%
  ...
  coord_flip() +
  geom_jitter(alpha = 0.35) +
  facet_wrap(capital ~ ., scales = "free", ncol = 2) +
  labs(
    title = "Word count of responses by form topics per capital",
    subtitle = "A box plot distribution",
    caption = "*Please refer to recode table for label forms"
  ) +
  theme_ecuador1()
...
cp %>% 
  ...
  labs(title = "Median word count of responses per partner",
       subtitle = sprintf("The total median rate %s", median(cp$.responseWordCount))) +
  xlab("Partners") +
  ylab("Median value") +
...
ggplot(med.wc, aes(Month, median, color = partnerName)) +
  geom_line() +
  geom_point() +
  labs(title = "Word count per partner",
       subtitle = "Median word count of responses") +
  xlab("Month") +
...
cast.med.wc <- reshape2::dcast(med.wc, Month ~ partnerName, value.var = "median")
ts.med.wc <- xts::xts(cast.med.wc[, -1], order.by = cast.med.wc$Month)
dygraph(ts.med.wc, main = "Word count per partner") %>%
  dyHighlight(
