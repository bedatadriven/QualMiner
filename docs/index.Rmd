...
p <- partners %>% 
  ...
  mutate(percent = paste0(round(100 * freq/sum(freq), 0), "%"))
```

The table below shows partner count per each record:

+ *`r p[1,"percent",drop=T]`* of the records are from
**`r p[1,"partnerName",drop=T]`**.

+ Second, **`r p[2,"partnerName",drop=T]`** comes with a percentage of 
*`r p[2,"percent",drop=T]`*. 
...
+ The sub-partner **`r psb[1,"subPartnerName",drop=T]`** has 
*`r psb[1,"percent",drop=T]`* of the total reports among the all sub-partners.
...
*`r sum(PartnerSubProp %>% filter(partnerName %in% "ACNUR") %>% pull(freq))`* 
total responses of **ACNUR** is actually coming from **HIAS**.

+ **UNICEF** has more diversed partners in terms of reporting.
*`r PartnerSubProp%>%filter(partnerName%in% "UNICEF", subPartnerName %in% "HIAS")%>%pull(percent)`*
of responses of **UNICEF** comes from **HIAS**.
*`r PartnerSubProp%>%filter(partnerName %in% "UNICEF", subPartnerName %in% "UNICEF")%>% pull(percent)`*
of reporting comes from the **UNICEF** itself.

+ Under **PMA**, there are *`r nrow(PartnerSubProp%>%filter(partnerName %in% "PMA"))`* sub-partners. **HIAS** reports *`r PartnerSubProp%>%filter(partnerName %in% "PMA", subPartnerName %in% "HIAS")%>%pull("percent")`* of these records.
...
locations %>%
  ...
  tab_header(
    ...
    subtitle = "Reports in narratives"
  )
```
