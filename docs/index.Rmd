...
The document includes the source code used in the analysis. You can click on the
`Code` in the areas to see the related code. You can change this behavior
globally in the document either by choosing `Show all code` or `Hide all code`
...
form.table %>% 
  group_by(recordId) %>% 
  summarize(length = length(recordId))
...
## Which records do have a "direct_indirect" field and which don't have?

```

```{r data-source-preparation, paged.print=FALSE}
...
```{r child = 'analysis/text-analysis.Rmd', include=FALSE}
```
