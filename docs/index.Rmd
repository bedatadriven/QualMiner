...
author: "Metin Yazici <i>(BeDataDriven B.V.)</i>, Alexander Bertram <i>(BeDataDriven B.V.)</i>, Lucy Brinks <i>(BeDataDriven B.V.)</i>"
date: "`r Sys.Date()`"
output:
  html_document:
    includes:
...
The difference between percentages of the partners **`r p[1,"partnerName",drop=T]`** and
...
psb <- partners %>% 
  ...
  count(subPartnerName, name = "freq", sort = TRUE) %>% 
  drop_na() %>% 
  ungroup() %>% 
  mutate(prop = round(freq/sum(freq), 3))
```

The table below shows sub-partner count per each record.

+ The sub-partner **`r psb[1,"subPartnerName",drop=T]`** 
has `r xfun::n2w(psb[1,"freq",drop=T])`
records 
that is *`r scales::percent(p[1,"prop",drop=T])`* percent
of  the total records among the all sub-partners.

+ The remaining sub-partners have infinitesimal numbers in their responses.
...
We should focus on a subset of the reports. Narrative data is multi-line text
fields allowing users to enter long texts.
...
```{r count-response, fig.width=8, fig.height=8}
...
between response word count and description fields. Interestingly, the questions
in the form *F15*, which is *Protección_VBG*, has no description fields at all.

```{r, plot1, fig.height=7, fig.width=7}
...
description.field.wc %>% 
  ...
  ylab(NULL) +
  theme_ecuador1() +
  scale_color_discrete(name = "Has a description field")+
...
```{r}
description.field.wc.bg <- description.field.wc %>% select(labelFormsRecode, .responseWordCount)
ppd.colors <- c("#00BFC4", "#F8766D")
ppd <- description.field.wc %>% 
  ## made 'has description' column factor to modify facets.
  mutate(.hasDescription = factor(description.field.wc$.hasDescription, labels = c("TRUE", "FALSE"))) %>% 
  ggplot(aes(x = .responseWordCount, y = labelFormsRecode, colour = .hasDescription)) +
  geom_point(data = description.field.wc.bg,
             colour = "grey",
             alpha = 0.2) +
  geom_point() +
  geom_point(shape = 21, color = "black") +
  facet_wrap( ~ .hasDescription) +
  scale_colour_manual(values = ppd.colors) +
  theme_ecuador1(border = TRUE) +
  guides(colour = FALSE) +
  labs(
    title = "Word count of responses & description field",
  ...
  xlab(NULL) +
  ylab(NULL)
## Solution taken from: https://github.com/tidyverse/ggplot2/issues/2096#issuecomment-292971159
g <- ggplot_gtable(ggplot_build(ppd))
stripr <- grep('strip-t', g$layout$name)
k <- 1
for (i in stripr) {
  j <- which(grepl('rect', g$grobs[[i]]$grobs[[1]]$childrenOrder))
  g$grobs[[i]]$grobs[[1]]$children[[j]]$gp$fill <- ppd.colors[k]
  k <- k+1
}
grid::grid.draw(g)
```
