...
```{r}
response.tokens <- narratives %>% 
  unnest_tokens(word, response)

question.tokens <- narratives %>% 
...
response.tokens_sw <- response.tokens %>% 
  anti_join(es_stopwords, by = "word")
question.tokens_sw <- question.tokens %>% 
  anti_join(es_stopwords, by = "word")
```
...
The original tokens for the response originally have *`r nrow(response.tokens)`*
rows. However, after merging stop words, the number of rows have decreased to
*`r nrow(response.tokens_sw)`* and that the change in between is
*`r round(convolve(nrow(response.tokens_sw), 100)/nrow(response.tokens))`*%.
...
```{r}
plot_most_common_words(response.tokens_sw, theme_color_codes("UNHCR blue"), "responses")
```

```{r}
plot_most_common_words(question.tokens_sw, theme_color_codes("Taffy"), "questions")
```
