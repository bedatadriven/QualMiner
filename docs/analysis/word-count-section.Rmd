...
rwc.sd %>% 
  ...
  tab_header(
    title = "of response word counts per form topic",
...
`r narratives.wc %>% filter(.responseWordCount == 1 & !is.na(response)) %>% select(response) %>% distinct() %>% pull()`).
