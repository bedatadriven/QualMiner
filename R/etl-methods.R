...
get_query_element <- function(formId, field.code.names) {
  ...
  tbl <- do.call(rbind, lapply(seq_along(queryTable), function(i) {
    ...
    common.names <- list(
      Month = "Month$|\\w*Mes$",
      Activity = "Sector-Actividad-Indicador.actividad",
      Donor = "Parent\\.donante\\.org_nombre", # reference field!
      COVID = "\\w*COVID$",
      Implementation = "Parent\\.implementacion",
      Modality = "modalidad_impl\\.modalidad"
    )
    ...
    relative.names <- list(
      userName = "Parent\\.Usuario\\.Name",
      partnerName = "Parent\\.implementador\\.org_nombre",
      cantonName = "Parent\\.ubicacion\\.name",
      cantonParentName = "Parent\\.ubicacion\\.parent\\.name"
    )
